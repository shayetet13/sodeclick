...
class SocketManager {
  ...
  async connect(baseURL) {
    ...
    this.socket.on('disconnect', (reason) => {
      ...
      // อัพเดท online status เป็น false เมื่อ socket disconnect
      const token = sessionStorage.getItem('token');
      if (token) {
        fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000'}/api/auth/logout`, {
          ...
          console.log('🔴 Socket disconnect: Online status updated to false');
        }).catch((err) => {
          console.error('❌ Socket disconnect: Failed to update online status:', err);
        });
      }
