...
const RealTimeChat = ({ roomId, currentUser, onBack, showWebappNotification }) => {
  ...
  // ใช้ SocketManager ที่มีอยู่แล้ว
  useEffect(() => {
    console.log('🔌 RealTimeChat: Setting up socket connection...');
    ...
    if (window.socketManager && window.socketManager.socket) {
      ...
      console.log('🔌 RealTimeChat: Creating new socket connection');
    ...
    newSocket.on('connect', () => {
      ...
      console.log('🚪 Joining room:', roomId, 'with userId:', currentUser._id);
    ...
    newSocket.on('new-message', (message) => {
      ...
      setMessages(prev => {
        ...
        if (tempMessageIndex !== -1) {
          ...
          newMessages[tempMessageIndex] = { ...message, isTemp: false };
          return newMessages;
        }
      ...
      // Scroll ลงด้านล่างเมื่อมีข้อความใหม่
      setTimeout(() => {
  ...
  const handleSendMessage = () => {
    ...
    if (editingMessage) {
      ...
      const tempMessage = {
        ...
        isTemp: true // ระบุว่าเป็นข้อความชั่วคราว
      };

      // เพิ่มข้อความชั่วคราวใน UI ทันที
