...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const handleBeforeUnload = () => {
      ...
      if (token) {
        ...
        console.log('🔴 Browser closing: Online status updated to false');
      }
    };
  ...
  const logout = () => {
    ...
    if (token && navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000'}/api/matching/update-location`, {
            ...
            return fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000'}/api/auth/logout`, {
              method: 'POST',
              headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json'
        ...
        (error) => {
          ...
          // อัพเดท online status เป็น false แม้ GPS ล้มเหลว
          const token = sessionStorage.getItem('token');
          if (token) {
            fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000'}/api/auth/logout`, {
              ...
              console.log('✅ Logout: Online status updated to false (GPS failed)');
            }).catch((err) => {
      ...
      // อัพเดท online status เป็น false เมื่อไม่มี GPS
      const token = sessionStorage.getItem('token');
      if (token) {
        fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000'}/api/auth/logout`, {
          method: 'POST',
          ...
          console.log('✅ Logout: Online status updated to false (no GPS)');
        }).catch((err) => {
          console.error('❌ Logout: Failed to update online status:', err);
        });
      }
